...
void SimJoiner::getRawResultED(unsigned q, unsigned threshold)
{
	...
	//for(int i = 0; i < m_str2.size(); ++ i) {
					...
					if(m.find(sub) != m.end())
						for(auto & r : m[sub])
							if(map.find(r) == map.end()) {
								m_rawResultED.push_back(make_pair(i, r));
								map[r] = true;
							}
				}
				++ q;
		...
		int k = 0;
		while(k < m_sstr.size()) {
			m_rawResultED.push_back(make_pair(i, m_sstr[k]));
			++ k;
		}
		++ i;
	}
...
int SimJoiner::joinED(const char *filename1, const char *filename2, unsigned q, unsigned threshold, vector<EDJoinResult> &result)
{
	...
	//get the raw result(for each one in m_str2)
	getRawResultED(q, threshold);

	//eliminate the false positives
	EDJoinResult r;
	vector<int> d0(m_len1), d1(m_len2);
	for(auto & p : m_rawResultED)
	{
		int ed = getED(m_str1[p.second], m_str2[p.first], threshold, d0, d1);
		if(ed <= threshold) {
			r.id1 = p.first;
			r.id2 = p.second;
			r.s = ed;
